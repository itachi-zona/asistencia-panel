...
HORA_LIMITE = (7, 49)   # hasta 7:49 = Asistencia; 7:50 en adelante = Tardanza
HORA_FALTA  = (16, 0)   # a partir de las 4pm se marcan Faltas automáticamente
...
def get_alumnos_usuario(uid):
    rows = db_exec(
        "SELECT nombre, genero FROM alumnos WHERE usuario_id=%s ORDER BY nombre ASC",
        (uid,), fetch="all"
    ) or []
    return rows
...
def generar_excel(uid, anio, mes, salon="", institucion="", profesora="", logo_b64=""):
    ...
    # ── Determinar qué días ya pasaron (para marcar F automáticamente) ────────
    hoy = info_hoy()
    hoy_total = hoy["ano"] * 10000 + hoy["mes"] * 100 + hoy["dia"]
    mes_actual = (anio == hoy["ano"] and mes == hoy["mes"])
    # Un día "cerrado" es: pertenece al pasado, o es hoy y ya pasó la hora de falta
    def dia_cerrado(d):
        if anio < hoy["ano"]: return True
        if anio > hoy["ano"]: return False
        if mes  < hoy["mes"]: return True
        if mes  > hoy["mes"]: return False
        if d    < hoy["dia"]: return True
        if d    > hoy["dia"]: return False
        # d == hoy["dia"]
        return es_hora_falta()
    ...
    for idx, nombre in enumerate(ALUMNOS):
        ...
        for i, d in enumerate(dias):
            ...
            if reg:
                ...
                # Día pasado sin registro → Falta automática en el Excel
                cell.value = "F"
                cell.font  = Font(bold=True, size=11, color="9C0006")
                cell.fill  = PatternFill("solid", fgColor="FFC7CE")
            else:
